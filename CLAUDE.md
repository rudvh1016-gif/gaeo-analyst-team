...
6. **테마 시스템(2026-07-18 대개편)**: 기본이 🖥️ 다크 터미널. head 인라인 스크립트가 첫 페인트 전에
   `html.gdark` 클래스를 붙이고(localStorage `gaeo_theme`, 기본 dark), `:root` 변수 세트를 `html.gdark`가
   통째로 교체한다. **본문 텍스트 색은 반드시 변수(--ink/--t2/--t3/--dim)로** — 하드코딩 slate hex를 쓰면
   다크에서 안 보인다. --navy는 다크에서 "밝은 액센트"로 뒤집히므로, --navy를 배경으로 쓰는 요소를
   새로 만들면 `html.gdark` 개별 오버라이드(딥블루 고정)를 함께 추가할 것. 파스텔 알약(연녹/연노랑 hex 배경)도
   동일 — rgba 저채도 틴트 오버라이드 필요. 우상단 `#siteTheme` 버튼이 토글(☀️/🌙 캔버스 버튼과 별개).
   숫자 서체는 `--num`(모노) — 티커·현재가·점수·KPI에 적용됨.
