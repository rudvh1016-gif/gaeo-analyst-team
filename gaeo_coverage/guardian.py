...
# 판정에 센다.
#   · 창 없이 '이번에 새로 사라진 것'만 세면 다음 날부터 0이 되어 가드가 죽는다.
#   · 누적으로 세면 정상 운영만으로 임계에 닿아 교체가 영구 정지한다.
# ⚠️ 창은 **상폐가 확정될 수 있는 시점보다 길어야** 한다. 짧으면 장애로 사라진
#    무리가 확정되려는 바로 그때 가드가 이미 꺼져 있다.
#    확정 최단 시점 = max(경과 10거래일 ≈ 14일, 서로 다른 날짜 3일 관측).
#    운영은 주 1회 실행이라 뒤쪽이 21일이 되므로 확정 최단은 21일이다.
#    21일로 두면 창 만료와 확정 가능 시점이 겹치는 칼날 경계였다
#    (2026-08-26 퀀트 6차 감사 지적 — 주 1회 리듬으로 실측해 확인).
#    여유를 두어 35일로 잡는다.
MASS_ABSENCE_WINDOW_DAYS = 35
