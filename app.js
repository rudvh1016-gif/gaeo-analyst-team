/* ============================================================
// 인라인 SVG 아이콘 중 일부 path 는 Lucide(ISC) · Feather(MIT) 아이콘 세트와 같다 — 라이선스·저작권 고지는 THIRD_PARTY_NOTICES.md (2026-09-16 준법 감사).
   개오 애널리스트팀 — 메인 앱 스크립트
...
/* 🧾 성적표 화면 코드는 scorecard-ui.js로 옮겼다 (2026-09-06, 약 88KB).
   지연 로딩이라 성적표/기록 화면을 열 때만 받는다 — index.html GaeoFeatures 'history' 키.
   아래 두 줄(LB_IDS·scoreStance)은 월간 캘린더도 쓰므로 여기 남는다. */
...
(function(){
  ...
  function setMode(mode){ // 'single' | 'watch' | 'guide' | 'compare' | 'portfolio' | 'news' | 'study' | 'lesson' | 'scorecard' | 'calendar' | 'community'
    ...
    // 성적표 코드는 scorecard-ui.js(지연 로딩)에 있다. 도착 전 클릭·경합을 대비해 존재할 때만 부른다.
    if(scorecard && typeof renderScorecard==='function') renderScorecard();
