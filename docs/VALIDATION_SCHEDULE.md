...
- 저장 실패는 세 가지를 구분해 적는다(2026-09-18): ① main 저장 성공 ② 커밋은 됐으나 push 실패 → 같은 커밋을 validation-inbox-<run> 브랜치에 보존하고 다음 실행이 --reconcile-inbox 로 회수한다(재채점 0) ③ 커밋 전에 멈춤 → inbox 브랜치가 없어 artifact 만 남고 자동 회수 경로가 없다(사람이 내려받아 넣는다). 실행기 크래시도 FAILED 로 기록한다.
