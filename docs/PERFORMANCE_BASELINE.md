...
- Service worker cache is `gaeo-shell-v26` (`v20` at this PR 3 baseline; bumped several times on 2026-09-03/04 together with `app.js`/`app-shell.css` as further home-layout changes touched both shell assets — most recently `?v=20260904-p9` when the always-floating privacy-settings button was un-fixed and moved into the footer, and the watchlist star button was restored to the site's own 44px touch-target floor); the exact query-versioned `app.js` and `app-shell.css` URLs are in the 29-entry shell and are cache-first only on an exact version hit. A new version miss is fetched and stored under its exact key, with an ignore-query last-good response allowed only after network failure. Navigation, data, and other changeable JS/CSS remain no-store network-first. No unbounded runtime cache or speculative prefetch was added.
