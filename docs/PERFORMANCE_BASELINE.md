...
- Service worker cache is `gaeo-shell-v31` with `?v=20260906-p16` (`v20` at this PR 3 baseline; bumped several times on 2026-09-03/04 together with `app.js`/`app-shell.css` as further home-layout changes touched both shell assets; `p13`/`v28` on 2026-09-04 when the warm-tinted `--color-accent-soft` token was made neutral; `p14`/`v29` on 2026-09-05 after the health check found six `app.js`/`app-shell.css` commits (analyst honesty, BUY surge warning, weight shrinkage) had shipped without a bump, so returning visitors with the service worker kept the pre-PR-#501 shell; `test_performance_budget.js` now also asserts the document `?v=` and the service-worker `?v=` are the same value; `p10`/`p12` are skipped because the service-worker contract test reserves those suffixes for its version-miss probe; `p15`/`v30` on 2026-09-06 when the home call-note TDZ bug was fixed, the scorecard entry link was added, and the analytics entry-cluster/scorecard-view/return-gap instrumentation shipped in `app.js` and `app-shell.css`; `p16`/`v31` on 2026-09-06 for the scorecard first-entry scroll fix and the computed SELL-share sentence); the exact query-versioned `app.js` and `app-shell.css` URLs are in the 29-entry shell and are cache-first only on an exact version hit. A new version miss is fetched and stored under its exact key, with an ignore-query last-good response allowed only after network failure. Navigation, data, and other changeable JS/CSS remain no-store network-first. No unbounded runtime cache or speculative prefetch was added.
...
|---|---|---|---|---|
| 2026-09-06 | (no budget change) | 832,000 / 320,000 / 90,000 unchanged | `app.js` 742,514B → 745,762B (+3,248B, +0.44%) | Home holiday false-alarm fix plus the stock-hero reorder. `GAEO_KRX_HOLIDAYS` and the three trading-day helpers were added at the **top** of `app.js` (a later position put them in the temporal dead zone for `renderClock`, which runs synchronously during load and killed the whole app once during this change); `snapshotStaleDays` now walks back to the previous trading session instead of only skipping weekends and pre-open hours. The verdict hero puts the stock name and price on the first line, drops the call from 34/48/38px to 30/32/30px, and adds a record line that calls the same `gaeoCallNoteHTML` the home brief uses. Headroom after: `app.js` 86,238B, `app-shell.css` 6,108B, `index.html` 4,233B. `?v=20260907-p20` / `gaeo-shell-v35`. No route transfer, DOM, LCP, or CLS budget changed. |
| 2026-09-06 | (no budget change) | 320,000 unchanged | `app-shell.css` 308,918B → 313,315B (+4,397B, +1.4%) | Scorecard redesign: a three-line lede, three collapsible groups, and a 12.5px mobile prose floor (measured 70.5% of scorecard text was below it at 390px; now 0% below 12px and 88.6% at 12.5px or above). The CSS budget stays at 320,000 with 6,685B headroom. No JS budget change — the new markup and the day-block bootstrap live in the lazily loaded `scorecard-ui.js` (91,622B → 98,446B), which is not in the service-worker shell and is fetched only when the scorecard or calendar is opened, so home first-paint transfer is unchanged. Measured collapsed scorecard height 11,986px → 2,857px at 390px and 7,446px → 1,649px at 1280px; expanding every group restores 13,314px / 8,041px with all 11 original blocks and 8 tables present. `?v=20260907-p19` / `gaeo-shell-v34`. No route transfer, DOM, LCP, or CLS budget changed. |
| 2026-09-06 | (no budget change) | 832,000 unchanged | 831,858B → 742,514B (−89,344B, −10.7%) | Scorecard screen code extracted from `app.js` into a lazily loaded `scorecard-ui.js` (91,622B). The previous row left 1,849B of headroom and said the next change should extract code rather than raise the ceiling again; this is that extraction, so the budget stays at 832,000 and headroom returns to 89,486B. Four contiguous line ranges (analyst rows + scorecard header/helpers, market-median helpers, model lab + model board + metrics, `renderScorecard` + its delegated `#scorecardView` listener) moved verbatim — each range was verified byte-identical inside the new file, and the only residual `app.js` edits are a pointer comment and a `typeof renderScorecard==='function'` guard. `LB_IDS`, `scoreStance`, `let SC_WEEK_OFFSET` and `goToGuideSection` stay in `app.js` because the monthly calendar, the deep-link router and the guide jump use them. No `window.*` export was added: both files are classic scripts, so `scorecard-ui.js` reads and writes `app.js` top-level `const`/`let` by bare name. Wiring is the GaeoFeatures `history` key (shared with the calendar, so the 13.6MB `history.js` is still fetched once). Measured home first-paint transfer 2,717.2KB → 2,630.4KB (−86.9KB) at 390px with request count unchanged at 52; rendered `#scorecardView` innerHTML byte-identical (47,028B) and innerText identical at 390px and 1280px against `origin/main`. Service worker shell stays 28 entries — lazy UI bundles are not precached, following `rotation-ui.js`. `?v=20260907-p18` / `gaeo-shell-v33`. No route transfer, DOM, LCP, or CLS budget changed. |
