...
- Service worker cache is `gaeo-shell-v21` (`v20` at this PR 3 baseline; bumped on 2026-09-03 together with `app.js`/`app-shell.css?v=20260903-p4` when the home menu relocation changed both shell assets); the exact query-versioned `app.js` and `app-shell.css` URLs are in the 29-entry shell and are cache-first only on an exact version hit. A new version miss is fetched and stored under its exact key, with an ignore-query last-good response allowed only after network failure. Navigation, data, and other changeable JS/CSS remain no-store network-first. No unbounded runtime cache or speculative prefetch was added.
