...
- Service worker cache is `gaeo-shell-v27` (`v20` at this PR 3 baseline; bumped several times on 2026-09-03/04 together with `app.js`/`app-shell.css` as further home-layout changes touched both shell assets — most recently `?v=20260904-p11` when the owner asked for the underlined home links to be replaced with quiet outlined chips and bold text, and the duplicated tan-tinted radar summary strip was removed; `p10` was skipped because the service-worker contract test already reserves that suffix for its version-miss probe); the exact query-versioned `app.js` and `app-shell.css` URLs are in the 29-entry shell and are cache-first only on an exact version hit. A new version miss is fetched and stored under its exact key, with an ignore-query last-good response allowed only after network failure. Navigation, data, and other changeable JS/CSS remain no-store network-first. No unbounded runtime cache or speculative prefetch was added.
