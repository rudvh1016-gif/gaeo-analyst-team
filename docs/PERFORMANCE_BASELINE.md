...
|---|---|---|---|---|
| 2026-09-06 | `assets.appJsRawBytes` | 828,000 → 832,000 | 827,499B → 829,156B (+1,657B, +0.20%) | Home call-note TDZ fix (`COVERAGE_LABEL` moved inside `gaeoCallNoteHTML`), the scorecard entry link under the note, and three measurement additions the strategy review asked for before repositioning the home BUY list: `stock_analysis_open.entry_cluster` with `stock_search_submit` no longer fired for non-search entries, `scorecard_view`, and `return_visit.visit_gap_bucket`. Ten dead `gaeoTrack('select_content', …)` calls (never sent: the event was not in the allowlist) were deleted first (≈850B); +1,657B net remained. The ceiling leaves ≈2.8KB headroom. No route transfer, DOM, LCP, or CLS budget changed. |
