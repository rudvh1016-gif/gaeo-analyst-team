...
⚠️ **좀비 판정에 elapsed time만 쓰지 마라 (2026-09-08).** "러너를 잡은 지 60분이 넘은
in_progress run"은 좀비 **후보**일 뿐이다. 실제 상태는 셋으로 갈린다 — (a) 정말 멈춤,
(b) 그냥 느림, (c) **수집은 되는데 저장(push)이 안 됨.** elapsed time은 셋을 구분하지 못한다.
2026-09-08에 실측된 것은 (c)였다: 러너가 GitHub과의 통신을 잃은 채 600종목 수집만 5사이클
(약 62분) 성공하고 push는 전부 실패해, run은 `in_progress`인데 산출물은 하나도 안 나왔다.
감시는 "돌고 있다"고 보고 사람은 "죽었다"고 봤는데 **둘 다 진상을 몰랐다.**
