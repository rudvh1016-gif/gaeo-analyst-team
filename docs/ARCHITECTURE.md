...
⚠️ **좀비 판정에 elapsed time만 쓰지 마라 (2026-09-08 오진).** "러너를 잡은 지 60분이
넘은 in_progress run"은 좀비 **후보**일 뿐이다. 2026-09-07·09-08에 나는 이 규칙만 보고
정상 작동 중인 시세 수집기를 두 번 취소했다. 로그를 나중에 읽어 보니 러너는 600종목을
전부 수집해 `data.js`까지 쓴 직후였고, `git push`만 남은 상태였다. 그날 그 사이클이
평소 11분이 아니라 74분 걸렸을 뿐이다. **산출물이 안 나온다는 것은 일을 안 한다는 증거가
아니다.** 경위는 `docs/PIPELINE_WATCHDOG.md`의 2026-09-08 절에 있다.
