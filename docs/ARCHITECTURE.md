...
방문자 전체에게 보이는 공개 게시판이다. 백엔드로 **kvdb.io**(무료 외부 key-value 스토어)를
쓰는데, 글 수정/삭제 비밀번호 확인이 **클라이언트 JS에서만** 이뤄진다 — 서버 쪽 인증이
없다. 즉 kvdb 버킷 주소를 찾아낸 사람은 비밀번호 확인을 우회해서 직접 `fetch`로 글을
수정/삭제할 수 있는 이론적 위험이 있다. 다만 영향 범위는 게시판 콘텐츠로 한정되고(사이트의
다른 데이터는 전부 git으로 관리되는 정적 파일이라 무관), 게시글 텍스트는 `esc()`로
HTML 이스케이프돼 저장되므로 저장형 XSS 경로는 없다.

## 관리자 발행 기능 ("🌍 모두에게 발행")

사이트 소유자가 자기 GitHub 개인 액세스 토큰을 관리자 모드 화면에 직접 입력하면,
index.html의 `ghPut(path, content, token)` 함수가 브라우저에서 바로
`api.github.com/repos/.../contents/...`를 호출해 파일을 커밋한다. 이 토큰은
**localStorage에 저장되어 브라우저에 남는다**(메모리에만 있다가 사라지는 게 아니다) —
그래서 이 토큰이 유출되면(공용 컴퓨터 사용, 악성 브라우저 확장 등) 그 사람이 사이트를
마음대로 바꿀 수 있다. 개인 기기에서만 쓴다면 위험은 낮지만, 더 안전하게 하려면 이 저장소
하나에만, Contents 읽기/쓰기 권한만, 만료일이 있는 GitHub fine-grained PAT를 쓰는 게 좋다
(아직 실제로 적용되진 않은 권장 사항).
