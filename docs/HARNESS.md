...
- 데이터 출처 준법·대체 공급망(2026-09-16): 판정표 `docs/legal/SOURCE_COMPLIANCE_MATRIX.md` · 네이버 의존성 지도 `docs/legal/NAVER_DEPENDENCY_MAP.md` · 대체 공급망 판정표 `docs/legal/SOURCE_REPLACEMENT_MATRIX.md` · 기계용 `config/source_compliance.json` `config/data_supply_migration.json` · 검사 `python3 legal_source_gate.py` · 그림자 비교(오프라인·Production 무변경) `python3 -m data_supply.shadow_compare`
