...
| FLOW(수급) 산식 6-arm 검증 | △ 표본 조건만 `check_flow_validation_readiness.py`. **arm 채점 코드 없음**(A0 재계산·B1/B2/C1/R 방향 문턱·분위 통계량·국면 3종 정의 미확정 — `config` definitionGaps 4건) | ✔ 표본 조건은 `VS-20260923`(9/23) | ✗ | ✗ 공통 날짜 **12 / 20**(익은 자동 판단일 34, flow_history 20일 8/11~9/8), 국면 조건 충족 1종(down_high 4일) / 3종 | ✗ | 9/15 실측 공통 16/20일·국면 2/3종(예상 충족 9/30). 정의 4건 제안 초안 = `docs/FLOW_6ARM_PREREGISTRATION_DRAFT.md` (**OWNER REVIEW REQUIRED** — 표본이 차기 전에 확정해야 사전등록이다) |
...
| 모의투자(PAPER) 성과 | ✔ `paper_engine.py` 등 · 테스트 20여 파일 | △ Windows 러너 단일 writer — **9/2 이후 정지**(원격 이력 재작성 → 러너 clone 공통 조상 상실) | ✔ `paper_trading/`(마지막 사이클 2026-09-01 15:05) | ✗ 8거래일 공백 + 관찰 기간 짧음 | ✗ | **RETIRED 2026-09-15** — 은퇴했다. 되살릴 관문이 없다. 원장(`paper_trading/`)은 보존되고 은퇴 뒤 신규 기록은 `ops_status`가 장애로 잡는다 |
| 기업행사·시장조치 증거(채점의 전제) | ✔ `collect_corporate_action_evidence.py` · `collect_kind_market_action.py` · `comparison_evidence.py` | △ **수집 워크플로에 예약 없음**(`workflow_dispatch` 단독) — 소비자는 30분마다, 생산자는 손으로만 | ✔ `gaeo_coverage/*_evidence.json` (마지막 2026-09-12) | ✗ 9/15 실측 DART 120종목·KIND 80종목 **전량 TTL 만료**, 판단 1,800건 전부 `unknown` | ✗ `comparable` 0건 | 2026-09-15 `ops_status.check_comparison_evidence` 로 **보이게** 만들었다(자료 부족). 갱신 주기·가격증명 경로는 `docs/operations/DATA_GAP_FINDINGS_20260915.md` §3 |
| 공식 결과가격 증명(채점의 두 번째 전제 · PHASE 4) | ✔ `krx_openapi_client.py` · `price_proof_planner.py` · `collect_price_proof.py` · `test_price_proof_producer.py` 35건 + 고의 파괴 12건 | △ `price-proof.yml` **수동 dispatch 만**(예약 0) · 인증키 Secret `KRX_OPENAPI_AUTH_KEY` **미등록** | ✗ 증명 0건(`research_archive/decisions/price_proofs/` 없음) | ✗ 신규 출처 판단 0건(9/16 자연 실행 뒤 시작) · 과거 1,800건은 `MISSING_PROVENANCE` | ✗ 합성 사슬 1건만 통과 | 소유자가 키를 등록 → dispatch → 첫 `comparable` → 기존 채점. 상세 `docs/operations/PRICE_PROOF_PRODUCER_20260916.md` |
| 실제 판단 성적표 한 장(`real_outcome_scorecard.py`) | ✔ **이번 추가** 얇은 조립층(새 채점기 0) · `test_real_outcome_scorecard.py` 21건 | ✔ 읽기 전용 CLI · `gaeo_check investment-contract` | ✔ `model_scoreboard.js` 를 읽는다 | ✗ 현재 모델 판단일 17/20 | ✗ `INSUFFICIENT_EVIDENCE` 가 정상 출력이다 | 판단일 20일 도달 · 거래비용 가정 등록(`config/scorecard_cost_assumption.json`, 소유자 결정 4건) |
