...
| `test_price_provenance.py` (신규) | §11 18개 시험 + 어휘 위장 방지 (32건) |
...
* `python3 -m unittest test_price_provenance -q` → 32건 통과
* **가드 6개를 하나씩 고의로 깨뜨려** 검사가 실제로 잡는지 확인 후 원상복구 — 6/6 잡음.
...
덕분에 `update-analysis.yml` 큰 `run:` 블록 여유가 **204B → 689B** 로 늘었다.
