...
## 2026-09-19 후속 — 키 없이 가능한 예정 검증 복구 경로 보완 (Codex)

사용자 범위: 해외 체류로 인증키 작업 불가. 키가 필요한 전환은 보류하고, 현재 코드로 재현되는 운영 결함을 우선 수리한다.
시작 기준: PR #592 병합 `b147a0f58ecabce7f69332da1de54014d6d746eb`. 예약·투자 산식·판정 기준을 추가/변경하지 않는다.

### 확인하고 고친 결함

- inbox 복구가 압축 입력(`.inputs.json.gz`)을 먼저 문자열로 읽어 `UnicodeDecodeError` 로 중단됐다. 모든 Git blob 을 바이트로 읽는다.
- fetch·파일 읽기·원장 손상·연결 파일 누락을 건너뛰고 성공으로 표시할 수 있었다. 전체 묶음과 원장 대응을 먼저 확인하고,
  파일을 완전히 저장한 뒤 원장에 추가한다. 원장 행이 없는 결과도 실패로 남겨 같은 시험의 재실행을 막는다.
- 수동 artifact 복원은 결과 JSON 만 옮겼다. 같은 회차의 압축 원본·후속 명세를 함께 복원하며, 누락·손상·다른 결과와 충돌·허용 폴더 밖 경로는 거부한다.
- inbox 에 함께 보관된 공식시세를 이 복구기가 옮기지 않았는데도 브랜치를 지울 수 있었다. 해당 커밋이 변경한 공식시세가 아직 보존되지 않았으면
  오류를 내고 브랜치를 남긴다. 최신 시세를 오래된 보관본으로 덮지 않으며, 이미 main 이력에 있는 커밋은 보존된 것으로 인정한다.
- 회수 실패 뒤에도 예정 시험을 실행할 수 있었다. 실제 기록 모드에서는 회수 성공 후에만 시험을 실행하고, 회수 실패를 최종 run 실패로 전달한다.

### 검증 근거

- 수정 전 최초 재현 10건 중 6 FAIL + 2 ERROR(기존 2건만 PASS). 압축 자료·누락 파일·결과 충돌·fetch 실패·수동 복원 결함을 재현했다.
- 수정 후 `test_validation_runner` **124건 PASS**(직전 108 + 신규 16). 임시 Git/보관본, 손상·저장 실패, 공식시세 보존,
  합성 자료의 원본 회수 → 같은 판정 replay → 중복 실행 없음까지 검증했다. 실제 봉인 평가를 조기 실행하지 않았다.
- `gaeo_check.py quick` 12개 파일 PASS. `test_workflow_size` + `test_ci_parity` 25건 PASS. `legal_source_gate.py` 0 findings.
- 별도 읽기 전용 검토에서 원장 행 누락 위험을 추가 발견·수정. 검토자는 실제 Git으로 누락 행 거부, 공식시세 삭제 불일치 보존,
  같은 삭제 인정, 이미 main 에 포함된 커밋의 최신 시세 유지까지 확인하고 추가 차단 결함 없음으로 판정했다.
- `gaeo_check.py premerge` **144개 파일 PASS / 0 FAIL**(Python 103 + JavaScript 41).
  GitHub CI·병합·Pages 결과는 이 항목을 포함한 PR 기록에서 확인한다. 실제 정기 운영 성공은 아직 미확인이다.

### 남은 일정·한계

- 현재 자연 실행 확인은 9/21 기존 ops-daily 부터다. 9/23 FLOW 표본 확인, 10/19 BUY 평가, 11/16 H1 확인은 기존 일정대로 둔다.
- 키가 필요한 공식 차트 실자료 확인과 만료된 외부 증거 확보는 보류. 이번 작업으로 투자 성과나 공식 채점 완료를 주장하지 않는다.
- 서로 다른 공식시세를 포함한 inbox 는 자동 덮어쓰기하지 않고 보존 오류를 알린다. 이미 전부 main 에 있어 변경 커밋이 없는 경우에도
  inbox 가 남을 수 있다(기존 동작). 데이터 유실은 없으며, 임의 삭제 대신 별도 확인 후 정리한다.
- 과거 원장·결과·사전등록·600종목·법적 게이트·실주문·유료 API·새 cron·LLM 반복 작업 변경 0. 현재 사용자가 해야 할 키 작업은 요청하지 않는다.

## 2026-09-19 밤 — 최신 작업 점검 + 저장 재시도·되읽기 보완 (Codex)

시작 기준: `origin/main 35c9dfb6839e164f2a9fed9efbd240975548b2fd`(fetch 및 preflight 일치).
최근 PR #591/#590/#586, 열린 PR·Issue, Actions 실행, 현재 main 산출물 및 실제 사이트 시세를 대조했다.
범위는 기존 운영의 부족분 수정이다. 새 판단 엔진·화면·공급자·예약을 만들지 않았다.

### 최신 작업에 남아 있던 실제 결함

PR #590 저장 수정과 #591 판정 전달 수정은 유지한다. 그 뒤 저장 확인 경로에서 세 가지 연결 결함을
**임시 Git 저장소의 실제 workflow run 블록으로 재현**했다(Production 유실 사고가 관측됐다는 뜻은 아니다).

1. push 거부 뒤 rebase 성공 시 커밋 SHA 가 바뀌는데 `committed` 출력은 옛 SHA 였다. 저장 성공을 미확인으로 오인할 수 있었다.
2. push 뒤 다른 수집기가 main 을 전진시키면 `ls-remote` 는 새 SHA 문자열만 돌려준다. 로컬에 객체가 없어 ancestor 검사가 실패했다.
3. main 포함을 확인하지 못해도 `pushed=true` 만으로 회수용 inbox 를 삭제했고, 저장 확인 실패는 최종 판정에 전달되지 않았다.

수정: rebase 직후 `committed` 갱신 → main 객체 fetch 후 포함성 확인 → `verified=true` 일 때만 기존 inbox 정리 →
`steps.saved.outcome` 실패를 마지막 판정에 전달. 조회 실패는 미확인으로 남기고 보관본을 보존한다.
기존 허용 경로·원장 append-only·과거 결과 수정/삭제 금지·PR #591 종료코드 계약은 그대로다.

### 현재 실측과 앞으로 할 일 (우선순위)

| 우선순위 | 현재 확인된 사실 | 다음 행동 / 완료 조건 |
| --- | --- | --- |
| P0 저장·판정 | #590 이후 수동 run `35363327604` 에서 main `f7516472` 저장 확인. #591 이후 ops-daily 실행은 조회 시점까지 없음 | 이번 저장 되읽기 수리 반영. 다음 9/21 정기 실행에서 점검 본문=전달 코드, 기록 저장·되읽기, 보존 결과를 확인. 수동/로컬 시험을 자연 실행 성공으로 쓰지 않는다 |
| P1 공식 일별 주가 | run `35198165790` 및 가장 최근 ops run 로그 모두 `SECRET_PRESENT=false`, `CREDENTIALS_MISSING`. 차트 스위치 OFF | 소유자가 승인된 `DATA_GO_KR_SERVICE_KEY` 를 GitHub Actions Secret 에 저장. 그 뒤 기존 `fsc-daily-price-verify` 1회 → 실자료 검증 성공 시 차트 전환 PR. 키 값은 채팅·로그·Git 에 쓰지 않는다 |
| P1 채점 근거 | 봉인 원본 누적 33,000행, 현재 요약 대상 3,600행 중 채점 0·결과 대기 3,600. DART 120/KIND 80 종목 증거는 전부 만료 | 결과시점 도달과 증거 확보를 분리한다. 기존 허용 게이트 범위에서 기업행사 증거를 갱신해야 한다. KRX 가격증명은 현재 법적 게이트도 닫혀 있어 키 하나만으로 해결되지 않는다. FSC 차트 자료도 현 채점 계약을 자동 통과하지 않는다 |
| P1 예정 검증 | 9/15 DIANA 확인 COMPLETED. 9/23 FLOW 표본, 10/19 BUY 평가, 11/16 H1 재확인은 NOT_DUE | 기존 `config/validation_schedule.json` 과 ops-daily 유지. 날짜 전 시험 실행·사전등록 조건 변경 없음. 도래 후 결과 파일과 main 저장을 함께 확인 |
| P2 성적 해석 | 9/18 성적표의 현재 모델: 20판단일, 전체 적중 64.0%, BUY 793건 적중 38.5%·비용 전 평균 −0.55%. 봉인 원장 공식 채점과 다른 집계 | 전체 적중률을 수익성으로 읽지 않는다. 비용·비교기준과 공식 증거가 갖춰지기 전 성능 개선을 주장하거나 산식을 최적화하지 않는다 |
| P2 자료 사용 범위 | 새 공급자 실전 전환 없음. KIND 탐색 PR #544 는 9/11 브랜치의 미병합 작업(파일 2개) | 이후 생긴 준법 게이트와 실제 중복 여부를 다시 검토하기 전 자동 병합/재실행하지 않는다. 네이버·수급·컨센서스 등 나머지 전환은 기존 공급망 표의 미해결 조건을 따른다 |
| P2 종목·연구 | Issue #557 의 마지막 주간 상태(9/13): 시세 599/600, 082640 UNKNOWN, 교체 제안 0. Evolution 후보 0·15판단일 | 다음 주간 산출물로 갱신 여부 확인. 종목 교체·상장폐지 확정·후보 승격을 추정하지 않는다. 현재 자동분석 600건과 최신 시세 확보 600건은 같은 뜻이 아니다 |
| 유지 | 9/18 시세 16:05·분석 16:10, 사이트 시세와 저장소 차이 0분. 주말 정상 대기. PAPER 는 은퇴 유지 | 주말 러너 재기동·PAPER 복원 불필요. update-analysis schedule 콜드스타트 실증은 여전히 별도 미확인 |

### 검증과 한계

- 수정 전 새 Git 재현 4건 모두 실패: rebase SHA 불일치, 전진한 main 오인, main 미포함 inbox 삭제, 조회 실패 inbox 삭제.
- 수정 후 `test_validation_runner` 108건 PASS(기존 102 + 신규 6). 실제 Git 경로 14건·최종 판정 8건 포함.
- `test_workflow_size` + `test_ci_parity` 25건 PASS. 별도 읽기 전용 검토자는 필수 수정 결함 없음으로 판정.
- `gaeo_check.py premerge` 144 PASS / 0 FAIL · `legal_source_gate.py` 0 findings. GitHub CI·병합 결과는 이 항목을 포함한 PR 기록에서 확인한다. **운영 정기 실행 검증은 아직 대기**다.
- 공식자료 인증키 등록은 소유자 조치다. 나머지 관찰은 기존 GitHub 예약의 산출물로 확인한다. 새로운 LLM 반복 작업은 만들지 않았다.
- BUY/HOLD/SELL 공식·가중치·임계값·사전등록·과거 원장·600종목 목록·법적 게이트·실주문·유료 API 변경 0.
