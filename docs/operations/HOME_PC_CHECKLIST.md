...
> # 🏁 2026-09-15 은퇴 — 이제 할 일이 **하나**로 줄었다
>
> 개오 애널리스트팀의 모의투자는 접었다(모의투자는 PRIVATE 사이트에서만 계속한다).
> 아래 「되살리기」 절차는 **더 이상 따라 하지 않는다** — 되살릴 때를 위한 참조본으로만 남긴다.
>
> ## 지금 집 PC 에서 할 일 (딱 하나)
>
> **작업 스케줄러에서 「GAEO Paper Trading」 을 해제(사용 안 함 또는 삭제)한다.**
>
> 1. 시작 메뉴에서 「작업 스케줄러」 를 연다.
> 2. 왼쪽 「작업 스케줄러 라이브러리」 를 누른다.
> 3. 목록에서 **GAEO Paper Trading** 을 찾는다.
> 4. 오른쪽 클릭 → **사용 안 함**(나중에 되살릴 수 있게 「삭제」보다 이쪽을 권한다).
>
> 안 해도 원장이 망가지지는 않는다 — 저장소의 게이트가 이미 모든 러너를 막고 있어서
> 매매 계산도, 기록도, 토스 토큰 요청도 하지 않는다. 다만 **30분마다 헛돌며 로그만 쌓이므로**
> 한 번 꺼 두는 편이 깔끔하다.
>
> 되살리는 법: `docs/operations/ORACLE_PAPER_MIGRATION_DECISION.md` §7
