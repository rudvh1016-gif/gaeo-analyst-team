...
>
> **3차(2026-09-03, 소유자 지시 "글자 크기 줄이고, 사이트 소개부터는 문단 내려서")**: 홈 히어로 글자를 줄이고
> "사이트 소개 · 데이터 출처와 면책 · 전체 글 목록" 링크를 설명 문단(`.hero-intro`) 밖 별도 줄(`.hero-trust`)로 옮겼다.
> 실측 font-size: 제목 `.hero-title` 데스크톱 1440 63→**44px**(clamp(34px,3.2vw,44px)) · 390 36→**26px** · 320 34→**24px**;
> 한 줄 소개 `.tagline` 17/16/15→**16/15/14px**; 설명 `.hero-intro` **15px 유지**(접근성 계약 ≥15px); 링크 줄 `.hero-trust` 13px(390 이하 12.5px),
> 설명 아래 8~10px 간격의 별도 줄. `index.html`의 `editorial-foundation.css?v=20260903-v1` → `v2`(브라우저 캐시 갱신용; 테스트·스냅샷은 이 문자열을 고정하지 않음).
> 스크린샷 `after/home-light-mobile390.png`·`home-light-desktop.png`·`home-dark-mobile390.png`는 3차 상태로 교체, `home-light-mobile320.png` 추가.
>
