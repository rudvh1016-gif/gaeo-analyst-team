...
## 8개 Skill (전부 `gaeo-` 접두어)

`/gaeo`를 입력하면 목록만 띄우는 안내 스킬(`.claude/skills/gaeo/SKILL.md`)이 실행돼 아래 8개를 표로 보여준다.
이 안내 스킬은 2026-08-20에 신설했다. 그 전에는 이 문서와 CLAUDE.md에 「`/gaeo`가 된다」고 적혀 있었지만
실제 스킬이 없어서 입력해도 아무 것도 안 떴고, 대표가 「스킬이 저장이 안 된 것 아니냐」고 오해하는 원인이 됐다.

⚠️ 이 스킬들은 저장소 안에 있어서 **이 저장소를 연 세션에서만 보인다.** 저장소를 열지 않은 일반 대화창에서는
안 뜨는 게 정상이다.
...
- "동시 2명 이내"의 뜻(2026-09-10 구간 E 명확화): **메인 작업자를 포함해 동시에 도는 AI 가 2개 이내**(메인 1 + 검토자 1). Codex 등 다른 도구에서도 같다.
- **같은 AI(같은 세션)가 역할만 바꿔 자기 결과를 검토하는 것은 독립 검토가 아니다.** 독립 검토로 치는 것은 ① 별도 세션의 검토자 1명(사람, 또는 별도로 띄운 AI 세션/서브에이전트) ② 기계 검사(`test_*`·`gaeo_check.py`)뿐이다. 서브에이전트가 없는 도구에서 관점을 순차로 바꿔 보는 것은 "자기 점검"이며, 중요 변경(보안·원장·투자 성적 의미·배포 경계)의 PASS 근거로 쓰지 않는다.
