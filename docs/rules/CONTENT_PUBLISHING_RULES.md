...
## 저작권·인용 — 공개 콘텐츠 전부에 적용 (2026-09-16 준법 감사에서 신설)

2026-09-16 실측: 본문 저장 0건 · 외부 이미지 0건 · 우회 도구 0건 · 최장 제3자 인용 93자 · 출처 링크 718건(그중 612건이 기사 제목을 `name`에 포함).
지금까지 깨끗했던 것은 스킬의 "자체 서술" 요구 덕분이었고 명시 규칙은 없었다. 아래를 규칙으로 고정한다.

1. **본문 저장·복제 금지.** 기사·공시·리포트의 문장을 그대로 옮기지 않는다. 사실만 취하고 GAEO 말로 다시 쓴다.
2. **인용 상한.** 제3자 발언 인용은 한 번에 2문장·100자 이내, 화자 표기 필수. `<blockquote>`·긴 따옴표 인용 금지.
3. **이미지 금지.** 기사 사진·언론사 로고·타 사이트 차트 캡처를 저장·표시하지 않는다. 그림은 자체 인라인 SVG 만.
4. **우회 금지.** 유료·로그인 기사를 캐시·아카이브·리더 모드로 읽지 않는다.
5. **출처 필드는 최소로.** `sources` 는 매체명·URL·발행일. 기사 제목은 식별에 꼭 필요할 때만 짧게. 제목·본문을 모아 검색 가능한 DB 로 만들지 않는다. 자동 수집(RSS·뉴스 API·검색 스크래핑)은 하지 않는다.
6. **사실과 해석 분리.** "기사가 전한 사실" 과 "개오 팀의 해석" 을 문장에서 구분한다.
7. **기존 글은 고치지 않는다.** 과거 글의 `sources.name` 에 제목이 들어간 것은 인용 표기로 두되, 새 글부터 5번 형식을 따른다.
8. **단정 법률 표현 금지.** "백 퍼센트 합법" 류의 문구를 코드·문서·화면에 쓰지 않는다(`legal_source_gate.py` 가 막는다). 상태 어휘만 쓴다.

근거·판정표: `docs/legal/SOURCE_COMPLIANCE_MATRIX.md`. 자동 검사: `python3 legal_source_gate.py` (premerge 포함).
