(function(global){
  ...
  function renderView(data,state){
    ...
    return `<div class="rot-shell">
      ...
      ${/* ⭐ 2026-09-04 소유자 지시(가독성): 카드 6개가 "권장 20거래일 기준"과
            "계산기간 2026.08.06~2026.09.04"를 각각 반복해서, 여섯 칸이 거의 같은
            글자로 채워져 있었다. 공통 사실은 위에 한 줄로 모으고, 카드에는
            그 카드에만 있는 것만 남긴다. 정보를 지우는 게 아니라 한 번만 쓴다. */''}
      <p class="rot-summary-basis">아래 요약은 <b>${recommendedPrefix}${summaryHorizon}거래일 기준</b>이고, 계산기간은 <b>${escapeHtml(summaryRange||'확인 중')}</b>입니다.</p>
      <section class="rot-summary" aria-label="순환매 요약">
        <article class="rot-card rot-card-lead"><span class="rot-card-context">현재 관찰</span><strong class="rot-card-primary">${leader.name?`${escapeHtml(leader.name)} 순환 신호`:escapeHtml(summary.headline||'뚜렷한 순환 신호 없음')}</strong>${meta([shortTermMeta])}</article>
        <article class="rot-card"><span class="rot-card-context">현재 1위 업종</span><strong class="rot-card-primary">${escapeHtml(leader.name||'확인 중')}</strong><p class="rot-card-secondary"><b>${number(leader.score).toFixed(1)}점</b> · ${signalLabel(leader.signal)}</p></article>
        ...
        <article class="rot-card"><span class="rot-card-context">다음 관찰 후보</span><strong class="rot-card-primary">${escapeHtml(candidate.name||'뚜렷한 후보 없음')}</strong><p class="rot-card-secondary"><b>${candidate.score!=null?number(candidate.score).toFixed(1)+'점':'조건 충족 업종 없음'}</b>${candidate.score!=null?' · 다음 후보':''}</p>${meta([observationMeta])}</article>
        <article class="rot-card"><span class="rot-card-context">시장 국면 · 최근 ${regimeDays}거래일</span><strong class="rot-card-primary">${escapeHtml(regime.direction||'확인 중')} · ${escapeHtml(regime.volatility||'확인 중')}</strong><p class="rot-card-secondary">${escapeHtml(regime.leadership||'시장')} 중심 · 최근 ${breadthDays}거래일 상승 종목 ${number(regime.breadthRate).toFixed(1)}%</p></article>
