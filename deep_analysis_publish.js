...
<script type="application/ld+json">${safeJson(article)}</script><script type="application/ld+json">${safeJson(breadcrumb)}</script><style>${publicPageStyle()}</style><link rel="stylesheet" href="/editorial-accessibility.css?v=20260903-v1"></head>
<body><a class="skip-link" href="#main-content">본문으로 바로가기</a><main class="da-shell" id="main-content" tabindex="-1"><header class="da-brand"><a class="da-wordmark" href="${baseUrl}">GAEO</a><span>Historical Research Record</span></header>
...
<footer class="da-footer-links"><a class="da-back" href="${hubUrl}">${escapeHtml(record.stockName)} 최신 판단·전체 기록 →</a><a class="da-back" href="${stockUrl}">현재 종목 분석 보기 →</a><a class="da-back" href="${archiveUrl}">전체 정밀분석 보기 →</a></footer></main></body></html>`;
}
...
function renderArchivePage(records, options = {}) {
  ...
  const hubSection = hubs
    ...
    : '';
  return `<!doctype html><html lang="ko"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><link rel="stylesheet" href="/assets/fonts/wanted-sans/WantedSansVariable.css"><title>${title}</title><meta name="description" content="GAEO가 특정 시점에 남긴 종목별 정밀분석 Historical Research Record를 최신순으로 확인하세요.${page > 1 ? ` 전체 기록 중 ${page}페이지입니다.` : ' 각 기록은 나중 판단으로 덮어쓰지 않고 날짜별로 보존합니다.'}"><link rel="canonical" href="${canonical}">${prevPath ? `<link rel="prev" href="${absoluteUrl(baseUrl, prevPath)}">` : ''}${nextPath ? `<link rel="next" href="${absoluteUrl(baseUrl, nextPath)}">` : ''}<style>${publicPageStyle()}${hubExtraStyle()}</style><link rel="stylesheet" href="/editorial-accessibility.css?v=20260903-v1"></head><body><a class="skip-link" href="#main-content">본문으로 바로가기</a><main class="da-shell" id="main-content" tabindex="-1"><header class="da-brand"><a class="da-wordmark" href="${baseUrl}">GAEO</a><span>Research Library</span></header><section class="da-index-head"><p class="da-kicker">HISTORICAL RESEARCH</p><h1>정밀분석 기록</h1><p>직접 지정해 더 깊게 확인한 종목의 당시 분석을 시간순으로 보존합니다.<br>각 기록은 최신 판단으로 덮어쓰지 않습니다.</p></section><div class="da-index-list">${rows.length ? rows.map((record) => `<a class="da-index-row" href="${absoluteUrl(baseUrl, record.permalink)}"><span class="da-index-date">${dotDate(record.analysisCreatedAt)}</span><span><span class="da-index-name">${escapeHtml(record.stockName)}</span><span class="da-index-sector">${escapeHtml(record.sector || record.ticker)}</span></span><span class="da-index-kind">정밀분석</span></a>`).join('') : '<p class="da-empty">최근 정밀분석이 아직 없습니다.</p>'}</div>${hubSection}<footer><nav class="da-pager" aria-label="정밀분석 기록 페이지 이동">${prevPath ? `<a href="${absoluteUrl(baseUrl, prevPath)}" rel="prev">← 이전 페이지</a>` : '<span></span>'}${nextPath ? `<a href="${absoluteUrl(baseUrl, nextPath)}" rel="next">다음 페이지 →</a>` : '<span></span>'}</nav></footer></main></body></html>`;
}
...
<meta property="og:type" content="article"><meta property="og:title" content="${escapeHtml(title.replace(' | GAEO', ''))}"><meta property="og:description" content="${escapeHtml(description)}"><meta property="og:url" content="${escapeHtml(canonical)}"><meta property="og:site_name" content="GAEO"><meta property="og:locale" content="ko_KR"><meta property="og:image" content="${baseUrl}gaeo-share-v3.jpg">
<meta name="twitter:card" content="summary_large_image"><meta name="twitter:title" content="${escapeHtml(title.replace(' | GAEO', ''))}"><meta name="twitter:description" content="${escapeHtml(description)}"><meta name="twitter:image" content="${baseUrl}gaeo-share-v3.jpg">
<script type="application/ld+json">${safeJson(article)}</script><script type="application/ld+json">${safeJson(breadcrumb)}</script><script type="application/ld+json">${safeJson(faqSchema)}</script><style>${publicPageStyle()}${hubExtraStyle()}</style><link rel="stylesheet" href="/editorial-accessibility.css?v=20260903-v1"></head>
<body><a class="skip-link" href="#main-content">본문으로 바로가기</a><main class="da-shell" id="main-content" tabindex="-1"><header class="da-brand"><a class="da-wordmark" href="${baseUrl}">GAEO</a><span>Research Record</span></header>
...
<footer class="da-footer-links"><a class="da-back" href="${stockUrl}">현재 종목 분석 보기 →</a><a class="da-back" href="${archiveUrl}">전체 정밀분석 보기 →</a></footer></main></body></html>`;
}
