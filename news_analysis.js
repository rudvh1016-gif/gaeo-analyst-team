...
const NEWS_ANALYSIS = [
  {
   ...
   "summary": "코스피는 +2.94%(6,772.22), 코스닥은 +0.24%(860.95)로 나흘 연속 상승했어요. 삼성전자 +3.72%·SK하이닉스 +5.52%가 지수를 끌어올렸지만 500종목 중 상승 240개·하락 242개로 반반이었어요. 전자·부품(MLCC)은 강했고 화장품·엔터는 약했어요.",
   "body": `## 딱 3줄 요약
- 코스피는 193.18포인트(+2.94%) 오른 6,772.22, 코스닥은 2.04포인트(+0.24%) 오른 860.95로 마감했어요. 나흘 연속 상승이에요.
- 밤사이 미국 반도체주가 일제히 강세를 보인 여파로 삼성전자 +3.72%, SK하이닉스 +5.52%가 지수를 끌어올렸어요. 외국인 약 5,200억원·기관 약 3,000억원이 순매수했어요.
- 그런데 500종목 중 오른 종목 240개, 내린 종목 242개로 사실상 반반이었어요. 어제(상승 234·하락 251)에 이어 이틀째 지수 상승률만큼 시장 전체가 좋았다고 보긴 어려운 하루예요.

## 이틀 연속 "지수는 오르는데 반은 빨간불"인 이유
어제 기사에서 코스피 3.68% 급등에도 상승 종목보다 하락 종목이 많았다는 이야기를 했었는데, 오늘도 비슷한 모양이 반복됐어요. 코스피는 시가총액이 큰 회사일수록 지수에 더 큰 목소리를 주는 계산 방식이에요. 반 평균 키를 구할 때 몸무게가 큰 학생 몇 명에게 훨씬 많은 표를 주는 것과 비슷해요. 삼성전자·SK하이닉스처럼 비중이 큰 종목 몇 개가 크게 오르면 나머지 종목이 다 같이 오르지 않아도 지수는 플러스로 나올 수 있어요.

오늘 500종목 표본에서는 240개가 오르고 242개가 내렸어요. 오히려 내린 종목이 2개 더 많아요. 코스피가 +2.94%나 뛰었는데 정작 내 종목이 빨간불일 확률이 절반을 살짝 넘는다는 뜻이에요. 이런 흐름이 어제오늘 이틀 연속 이어지고 있다는 게 핵심이에요. 반도체·전자부품이라는 특정 업종에 자금이 계속 몰리는 사이, 어제 업종 평균 +6.80%로 가장 강했던 화장품·미용은 오늘 -3.52%로 정반대로 뒤집혀 가장 많이 빠졌고 엔터·여행 업종도 약했어요.

이걸 시장 폭(breadth)이라고 부르는데, 이틀 연속 절반 안팎에 머물고 있다는 건 상승세가 넓게 퍼지지 못하고 특정 업종을 오가며 순환하고 있다는 신호예요. 코스피 숫자 하나만 보고 오늘 장이 다 좋았다고 판단하면 실제 내 계좌와는 다른 그림을 보게 될 수 있어요.

## 무슨 일이 있었나: 숫자로 보는 8월13일 시장
- 코스피는 전일 6,579.04에서 6,772.22로 193.18포인트(+2.94%) 올랐고, 코스닥은 858.91에서 860.95로 2.04포인트(+0.24%) 오르는 데 그쳤어요.
- 삼성전자는 25만 5,500원에서 26만 5,000원으로 +3.72% 올랐고, SK하이닉스는 150만 4,000원에서 158만 7,000원으로 +5.52% 올랐어요. 배경은 미국 반도체주 동반 강세예요. 마이크론 테크놀로지가 +4.92%, SK하이닉스 미국주식예탁증서(ADR)가 +9.01% 뛰었고, AI 클라우드업체 코어위브와 서버 제조사 슈퍼마이크로컴퓨터도 20%대 급등했어요. 오늘 외국인이 약 5,200억원, 기관이 약 3,000억원을 순매수하며 지수 상승을 뒷받침했어요.
- 전자·부품 업종은 평균 +2.68%로 오늘 가장 강했어요. 삼성전기가 8월1일 출하분부터 MLCC(적층세라믹콘덴서) 공급가를 30% 인상한다고 밝힌 여파로 +12.13%(149만 7,000원) 올랐고, 삼화콘덴서도 +17.39%(10만 3,300원)로 이틀 연속 급등했어요. 방산(+2.67%)·로봇(+2.18%)·자동차·부품(+1.49%)도 강했어요.
- 개별 상승률로는 코오롱티슈진이 +19.67%로 가장 컸지만, 이 종목은 뚜렷한 개별 재료가 확인되지 않아 특정 자금이 짧게 쏠린 결과로 보여요. 반대로 화장품·미용은 평균 -3.52%로 가장 약했어요. 어제 하루 만에 +6.80% 뛰었던 업종이라 차익실현 매물이 나온 것으로 보여요. 여행레저(-2.41%)·게임·엔터(-2.37%)도 약했는데, JYP엔터테인먼트가 2분기 영업이익 310억원으로 컨센서스(378억원)를 18% 밑도는 어닝미스를 낸 데다 트와이스 멤버 정연의 전속계약 종료로 재계약 불확실성까지 겹치며 -11.20%(4만 2,250원)까지 급락한 영향이 컸어요. 메리츠·SK·NH투자증권 등 복수 증권사가 목표주가를 6만9,000원~7만9,000원에서 6만5,000원~6만9,000원 수준으로 일제히 하향했어요.
- 개별 종목 중에는 어제 2분기 실적 서프라이즈로 급등했던 실리콘투가 -9.39%(4만 1,000원)로 되돌림했고, 연료비·전력구입비 부담 우려에 증권사가 목표주가를 하향한 한국전력은 -7.33%(3만 2,850원) 내렸어요. 건설·건자재(-1.39%)도 약해, 어제 급등했던 금호건설을 포함해 최근 급등했던 업종들이 오늘은 대체로 숨 고르기에 들어간 모습이었어요.

## 오늘 시장에 좋은 점 (호재 요인)
- **나흘 연속 상승이 이어졌어요.** 코스피는 8월10일부터 오늘까지 나흘째 오름세를 이어가고 있어요. 6,300선이던 지수가 나흘 만에 6,772선까지 올라온 셈으로, 급등 뒤 곧바로 큰 폭으로 되밀리지 않았다는 점은 긍정적이에요.
...
## 오늘 시장에 부담인 점 (주의할 요인)
- **코스피 +2.94%인데 상승 종목보다 하락 종목이 더 많았어요.** 500종목 중 상승 240개·하락 242개로, 어제(상승 234·하락 251)에 이어 이틀 연속 지수 상승률만큼 시장 폭이 넓어지지 않고 있어요.
- **어제 주도했던 업종이 하루 만에 가장 크게 빠졌어요.** 화장품·미용은 어제 +6.80%에서 오늘 -3.52%로 반전했어요. 단기간에 급등한 업종일수록 차익실현에 따른 되돌림이 클 수 있다는 걸 보여줘요.
- **개별 종목 단위의 실적 실망이 컸어요.** JYP엔터테인먼트는 어닝미스에 핵심 아티스트 재계약 불확실성까지 겹치며 -11.20% 급락했고, 어제 실적 호재로 급등했던 실리콘투도 -9.39%로 되돌림했어요. 지수가 오르는 날에도 개별 실적·재료는 그대로 주가에 반영된다는 걸 보여준 사례예요.
...
중기적으로는 반도체·전자부품 쏠림이 다른 업종으로 확산되는지가 관전 포인트예요. 이틀 연속 상승 종목 수가 절반 안팎에 머물고 있다는 건, 지수 상승의 온기가 여전히 일부 업종에 집중돼 있다는 뜻이에요. 화장품·엔터처럼 최근 크게 흔들린 업종이 진정되는지, 아니면 계속 순환매 대상이 바뀌는지 지켜볼 필요가 있어요.

코오롱티슈진처럼 개별 재료가 뚜렷하지 않은데 크게 튀는 종목은 뒤늦게 따라 들어가기보다 재료가 확인될 때까지 지켜보는 편이 안전해요.
...
- 시장 폭 확인: 이틀 연속 500종목 상승 수가 절반 안팎에 머물고 있어요. 코스피 상승률만 보지 말고 내 업종이 상승 쪽인지 따로 확인해요.
- 삼성전자·SK하이닉스: 오늘 상승분(각각 +3.72%·+5.52%)을 다음 거래일에도 지키는지, 전날 밤 미국 반도체주 흐름과 함께 확인해요.
...
- 한 줄 결론: 오늘 코스피 +2.94%는 반도체·전자부품에 집중된 상승이고 상승 종목보다 하락 종목이 더 많았어요. 오늘 밤 미국 CPI 결과와 함께 내 업종이 상승 쪽인지부터 확인하는 게 안전해요.`,
   "sources": [
