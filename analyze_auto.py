...
def quant_eval(e, t, qstats):
    ...
    f = [
        ...
        f"과거에 이런 상태({used})였던 적이 {b['n']}건 있었는데, 그중 {b['w']}건에서 5거래일 뒤 실제로 주가가 올랐어요 → 이 비율이 '경험적 승률' {wr:.0f}%예요",
        f"그 {b['n']}건의 5거래일 뒤 등락률을 다 더해 평균 낸 값은 {avg:+.1f}%예요(오른 경우·내린 경우 전부 포함 — 승률과는 다른 숫자) · 과거 통계일 뿐 미래를 보장하진 않아요",
    ]
    return {"score": score, "stance": stance_of(score), "findings": f[:4]}
