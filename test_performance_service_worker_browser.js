...
(async () => {
  ...
  try {
    ...
    const shell = await page.evaluate(async () => {
      ...
      const key = keys.find(value => value === 'gaeo-shell-v28');
    ...
    check(shell.key === 'gaeo-shell-v28', 'service worker v28 shell이 활성화됨');
    ...
    const versionUpdate = await page.evaluate(async () => {
      const cache = await caches.open('gaeo-shell-v28');
