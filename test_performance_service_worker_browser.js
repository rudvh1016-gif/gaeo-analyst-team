...
(async () => {
  ...
  try {
    ...
    const shell = await page.evaluate(async () => {
      ...
      const key = keys.find(value => value === 'gaeo-shell-v30');
    ...
    check(shell.key === 'gaeo-shell-v30', 'service worker v30 shell이 활성화됨');
    ...
    const versionUpdate = await page.evaluate(async () => {
      const cache = await caches.open('gaeo-shell-v30');
