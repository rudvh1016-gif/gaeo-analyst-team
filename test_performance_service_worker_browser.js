...
(async () => {
  ...
  try {
    ...
    const shell = await page.evaluate(async () => {
      ...
      const key = keys.find(value => value === 'gaeo-shell-v35');
    ...
    check(shell.key === 'gaeo-shell-v35', 'service worker v35 shell이 활성화됨');
    ...
    const versionUpdate = await page.evaluate(async () => {
      const cache = await caches.open('gaeo-shell-v35');
