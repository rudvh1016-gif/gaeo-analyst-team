...
- `gaeo`·`gaeo-*` — 저장소 개발·점검 스킬(`docs/gaeo_team_system.md`)

> Codex 등 다른 도구용 공용 진입점은 `.agents/skills/<같은 이름>/SKILL.md`에 있다(2026-09-10). 손으로 만들지 않고 `python3 sync_agent_compat.py`가 이 폴더의 원본에서 생성한다. 원본을 고치면 다시 생성할 것(`test_agent_compat.py`가 어긋남을 잡는다).
