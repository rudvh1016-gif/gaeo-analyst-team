...
> ⛔ 2026-07-23 사고: 사용자가 "금일 코스피 코스닥 종가 기준으로 아주 상세하게 업뎃진행해"라고만
> 했는데, 이걸 "정밀분석 14종목 전체 재작성"으로 확대해석해 실행했다(그 과정에서 캘린더 events
> 초기화 사고까지 파생). 사용자는 이런 대규모 작업을 시킨 적이 없다며 크게 항의했다.
> **정밀분석(analysis.js) 기존 종목 전체 재작성은 사용자가 "정밀분석/재분석/analysis.js 갱신"처럼
> 명시적으로 요청했을 때만 수행한다.** "업데이트해줘", "오늘 기준으로 반영해줘", "상세하게 해줘"
> 같은 모호한 지시만으로는 절대 이 무거운 작업(5인×13~14종목 전체 재작성)을 임의로 시작하지 않는다
> — 의도가 조금이라도 불분명하면 무엇을 갱신하길 원하는지(시세만? 시장분석만? 특정 종목만?
> 전체 재분석?) 먼저 사용자에게 확인한 뒤 진행한다.
- tickers.js는 **600종목**이지만 정밀분석(analysis.js) 대상은 **현재 analysis.js에 이미 있는 종목(13개 안팎)뿐**이다.
  "전부 분석해줘"라는 요청도 **정밀분석은 기존 목록 유지**가 기본 — 600종목 정밀분석을 시도하면 토큰이 폭발한다.
  나머지 580여 종목은 러너의 자동분석(auto_analysis.js)이 30분마다 알아서 갱신한다.
...
- **NOVA에는 `sources`를 함께 기록**(웹검색에서 실제 확인한 기사만): `"sources":[{"t":"기사 제목(짧게)","p":"언론사","d":"M/D","u":"<기사 URL>"}]` 1~2건 — URL(`u`)을 함께 남겨 출처를 따라갈 수 있게 한다(2026-09-16 준법 규칙). 기사 문장을 그대로 옮기지 않고, 인용은 2문장·100자 이내로 화자를 붙인다.
...
> **정밀분석이 자동분석을 항상 덮는다(우선, 단 '당일 + 시세 ±3% 이내'일 때만)**.
> ⚠️ 2026-08-10 정정: analyze_auto.py는 정밀분석 보유 종목을 건너뛰지 않는다 — **600종목 전부(정밀분석
> 종목 포함)를 예외 없이 계속 자동분석하고 auto_analysis.js에 채운다.** "정밀분석이 우선 표시된다"는 건
> 순전히 화면(index.html)의 프런트엔드 표시 우선순위일 뿐, 백엔드 자동분석 파이프라인 자체를 멈추거나
> 건너뛰게 하지 않는다. 즉 정밀분석한 종목도 시세는 10분마다, 자동분석은 약 30분마다 계속 갱신된다.
> tickers.js 전체를 매번 정밀분석할 필요 없음 — **핵심 종목만 정밀분석, 나머지는 자동분석에 맡겨 토큰을 아낀다.**
